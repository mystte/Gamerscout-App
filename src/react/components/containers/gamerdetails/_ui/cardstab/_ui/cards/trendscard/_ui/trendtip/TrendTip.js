...
const TrendTip = ({ gamertag, data, getColor }) => {
  ...
  const renderKdaRow = (id, value) => (
    <div style={styles.kdaRow}>
      ...
      {id === TRENDS_LINE_TYPE.KDA && <span>{gamertag}:</span>}
      {id === TRENDS_LINE_TYPE.TEAM_KDA && <span>{labels.teamAvg}:</span>}
  ...
  return (
    <div style={styles.container}>
      ...
      {data[1] && renderKdaRow(data[1].id, data[1].value)}
    </div>
  );
};
