...
const ToggleButton = ({ selected, onClick, type }) => {
  ...
  const title =
    type === APPROVAL_TYPE.DISAPPROVAL ? labels.disapprove : labels.approve;
  const icon = type === APPROVAL_TYPE.DISAPPROVAL ? 'thumb-down' : 'thumb-up';

  return (
    <button style={buttonStyle} onClick={() => onClick(type)}>
      <SVGIcon width={12} height={12} name={icon} />
