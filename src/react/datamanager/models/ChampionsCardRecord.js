...
export default class ChampionsCardRecord extends ExtendsWith(
  Record(defaultProps, 'ChampionsCardRecord')
) {
  static apiParser(apiData) {
    ...
    Object.keys(apiData.roles).forEach(key =>
      parsedData.positionsStatsList.push(
        PositionsRankRecord.apiParser({
          positionType: key,
          ...apiData.roles[key],
        })
      )
