...
export default class ConfigRegionRecord extends ExtendsWith(
  Record(defaultProps, 'ConfigRegionRecord')
) {
  static parseRegions(data) {
    const regionsList = data.regions_short
      ? data.regions_short.map(regionName => ({
          name: regionName,
