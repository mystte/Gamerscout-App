...
export default class HistoryListRecord extends ExtendsWith(
  Record(defaultProps, 'HistoryListRecord')
) {
  static apiParser(data) {
    const parsedData = {
      historyCards: data.map(recentGameData =>
        HistoryCardRecord.apiParser(recentGameData)
      ),
    };
