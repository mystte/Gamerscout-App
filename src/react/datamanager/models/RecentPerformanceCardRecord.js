  ...
  class extends superclass {
    ...
    getFilteredData(positionFilter, championFilter) {
      ...
      this.recentPerformanceList.forEach(recentPerfData => {
        ...
        const championExist =
          champions.find(champ => recentPerfData.champion === champ.name) !==
          undefined;
        if (!championExist) {
          champions.push({
...
export default class RecentPerformanceCardRecord extends ExtendsWith(
  Record(defaultProps, 'RecentPerformanceCardRecord')
) {
  static apiParser(data) {
    const parsedData = {
      recentPerformanceList: data.map(matchData =>
        RecentPerformanceDataRecord.apiParser(matchData)
      ),
    };
